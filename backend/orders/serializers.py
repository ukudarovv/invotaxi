...
class OrderSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        # Нормализуем телефон: убираем пробелы и проверяем, что не пустой
        if passenger_phone:
            passenger_phone = passenger_phone.strip()
            if not passenger_phone:  # Если после strip стала пустой
        ...
        if passenger_region_id:
            passenger_region_id = passenger_region_id.strip()
            if not passenger_region_id:
                passenger_region_id = None
        ...
        logger.info(f'Создание заказа: passenger_phone={passenger_phone}, passenger_name={passenger_name}, passenger_id={validated_data.get("passenger_id")}')
        ...
        def get_or_create_passenger(pickup_lat=None, pickup_lon=None):
            ...
            # Проверяем, что телефон указан и не пустой
            if not passenger_phone or (isinstance(passenger_phone, str) and not passenger_phone.strip()):
                logger.debug('Телефон пассажира не указан или пустой')
            ...
            try:
                ...
                # Регион необязателен в форме, но обязателен в модели, поэтому используем первый доступный
                if not region:
                    region = Region.objects.first()
                    if region:
                        logger.info(f'Использован первый доступный регион: {region.id}')
                    else:
                        # Если регионов нет в БД, создаем дефолтный регион
                        logger.warning('Не найдено ни одного региона в БД. Создаем дефолтный регион.')
                        try:
                            # Создаем или получаем дефолтный город
                            default_city, city_created = City.objects.get_or_create(
                                id='default',
                                defaults={
                                    'title': 'Атырау',
                                    'center_lat': 47.1167,
                            ...
                            if city_created:
                                logger.info(f'Создан дефолтный город: {default_city.title}')
                            
                            # Создаем дефолтный регион
                            ...
                            if region_created:
                                logger.info(f'Создан дефолтный регион: {region.title}')
                            else:
                                logger.info(f'Использован существующий дефолтный регион: {region.title}')
                        except Exception as e:
                            logger.error(f'Ошибка создания дефолтного региона: {e}', exc_info=True)
                            raise serializers.ValidationError({
                                'passenger_region_id': f'Не удалось создать дефолтный регион: {str(e)}'
                            })
                ...
                # Если регион все еще не найден, берем первый доступный или создаем дефолтный
                # Регион обязателен в модели, поэтому всегда используем первый доступный, если не указан
                if not region:
                    region = Region.objects.first()
                    if not region:
                        # Если регионов нет в БД, создаем дефолтный регион
        ...
        # Пытаемся создать пассажира до геокодирования
        # Вызываем функцию всегда, если передан passenger_phone (даже если регион и координаты не указаны)
        # Функция сама определит регион по координатам или возьмет первый доступный
        if not validated_data.get('passenger') and passenger_phone:
            ...
            logger.info(f'Попытка создать пассажира до геокодирования: phone={passenger_phone}, lat={pickup_lat}, lon={pickup_lon}')
            get_or_create_passenger(
                pickup_lat=pickup_lat if pickup_lat else None,
                pickup_lon=pickup_lon if pickup_lon else None
            )
        
        # Если passenger все еще не установлен, используем текущего пользователя
        # Но если передан passenger_phone, не выбрасываем ошибку здесь - создадим пассажира позже
        if not validated_data.get('passenger'):
            request = self.context.get('request')
            if request and hasattr(request.user, 'passenger'):
                validated_data['passenger'] = request.user.passenger
            # Если передан passenger_phone, пассажир будет создан позже (после геокодирования или с координатами)
            # Не выбрасываем ошибку здесь, если есть passenger_phone
        ...
        # Вызываем даже если координаты не переданы - функция сама определит регион
        if not validated_data.get('passenger') and passenger_phone:
            pickup_lat = validated_data.get('pickup_lat')
            pickup_lon = validated_data.get('pickup_lon')
            # Вызываем функцию создания пассажира с координатами (если есть) или без них
            get_or_create_passenger(
                pickup_lat=pickup_lat if pickup_lat else None,
                pickup_lon=pickup_lon if pickup_lon else None
            )
        
        # Финальная проверка пассажира
        if not validated_data.get('passenger'):
            logger.warning(f'Пассажир не установлен после всех попыток создания. passenger_phone={passenger_phone}, passenger_name={passenger_name}, passenger_id={validated_data.get("passenger_id")}, passenger_region_id={passenger_region_id}')
            if passenger_phone:
                # Если передан телефон, но пассажир не создан, это ошибка
                logger.error(f'Не удалось создать пассажира с телефоном {passenger_phone}. Проверьте логи выше для деталей.')
                # Проверяем, есть ли регионы в БД
                region_count = Region.objects.count()
                logger.error(f'Количество регионов в БД: {region_count}')
                if region_count == 0:
                    raise serializers.ValidationError({
                        'passenger_phone': 'Не удалось создать пассажира: в системе нет регионов. Пожалуйста, добавьте хотя бы один регион.'
                    })
                else:
                    raise serializers.ValidationError({
                        'passenger_phone': f'Не удалось создать или найти пассажира с телефоном {passenger_phone}. Проверьте данные (имя, категория инвалидности) и попробуйте снова.'
                    })
            else:
                # Если телефон не передан, проверяем текущего пользователя
                request = self.context.get('request')
                if request and hasattr(request.user, 'passenger'):
                    validated_data['passenger'] = request.user.passenger
                    logger.info(f'Использован пассажир текущего пользователя: {request.user.passenger.id}')
                else:
                    logger.error('Пассажир не указан и текущий пользователь не является пассажиром')
                    raise serializers.ValidationError({
